...
//
// WriteTimeout covers the whole response. A CLI run is allowed cliTimeout to
// produce it, so a WriteTimeout at or below cliTimeout does not stop attacks:
...
func TestWriteTimeoutOutlastsTheCLIBudget(t *testing.T) {
	if srvWriteTimeout <= cliTimeout {
		t.Fatalf("srvWriteTimeout = %s, cliTimeout = %s: the server would abort its own\nresponse while the CLI is still allowed to be producing it", srvWriteTimeout, cliTimeout)
	}
	...
	if margin := srvWriteTimeout - cliTimeout; margin < minMargin {
		t.Errorf("margin over cliTimeout = %s, want at least %s", margin, minMargin)
	}
}
